// Posters
import officePoster1 from '../../../assets/films/TheCorp/poster-first-aid-fail-the-office-us.webp';
import officePoster2 from '../../../assets/films/TheCorp/poster-fire-drill-the-office-us.webp';
import officePoster3 from '../../../assets/films/TheCorp/poster-parkour-the-office-us.webp';

// Videos
const theOffice1 = 'https://www.youtube.com/embed/Vmb1tqYqyII';
const theOffice2 = 'https://www.youtube.com/embed/gO8N3L_aERg';
const theOffice3 = 'https://www.youtube.com/embed/0Kvw2BPKjz0';

export const dataSliderTheCorp = [
	{
		itemPoster: officePoster1,
		itemSrc: theOffice1,
		itemAlt: `Poster of Dwight Schrute's unsettling first aid training in The Office.`,
		itemCaption: `First aid fails, human skin suits, and a truly unforgettable first aid session.`,
	},
	{
		itemPoster: officePoster3,
		itemSrc: theOffice3,
		itemAlt: `Poster capturing Jim, Dwight, and Michael's hilariously clumsy parkour attempts.`,
		itemCaption: `Parkour! Office athleticism at its most chaotic and cringe-worthy.`,
	},
	{
		itemPoster: officePoster2,
		itemSrc: theOffice2,
		itemAlt: `Poster of Michael Scott's infamous fire drill chaos in The Office.`,
		itemCaption: `Fire! Mayhem, panic, and a very intense fire drill.`,
	},
];
